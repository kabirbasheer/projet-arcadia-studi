...
export const Footer = () => {
  ...
  return (
    <FlowbiteFooter container={true} theme={customFooterTheme}>
      <div
        className={`${footerSize}  grid md:grid-cols-4 md:gap-x-7 gap-y-5 md:gap-y-0`}
      >
        <div className="grid grid-cols-1 md:content-start justify-items-center md:justify-items-start gap-y-5 p-5">
          ...
          <div className="flex items-center md:items-start flex-col md:gap-y-4 gap-y-3">
            {adressList.map((list, index) => (
              <div
                className={`flex items-center gap-x-3 text-primary-900`}
                key={index}
              >
                {list.icon && <list.icon />}
                <Typography
                  customClassName="text-base font-normal text-start text-slate-900"
        ...
        <div
          className={`grid md:justify-around md:content-start md:justify-items-start justify-items-center gap-y-4 p-5 ${footerWidth}`}
        >
          <FlowbiteFooter.Title
            theme={customFooterTheme.title}
            title="A propos"
          />
          <FlowbiteFooter.LinkGroup theme={customFooterTheme.groupLink}>
            {aboutList.map((list, index) => (
              <FlowbiteFooter.Link
                theme={customFooterTheme.groupLink.link}
                key={index}
                href={list.link}
              >
                <Typography
                  customClassName="flex items-center text-base font-normal"
                  fontFamily="secondary"
                >
                  {list.name}
                </Typography>
      ...
      <div className="max-w-screen-md md:w-[1180px] grid grid-rows-1 md:justify-start justify-center">
        <FlowbiteFooter.Copyright
          theme={customFooterTheme.copyright}
          by=" Arcadia Zoo"
